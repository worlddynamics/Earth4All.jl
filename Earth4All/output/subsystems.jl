...
function output(; name, params=_params, inits=_inits, tables=_tables, ranges=_ranges)
    ...
    @variables EDEFCA(t) [description = "ED Effect on Flow to Capacity Addition"]
    @variables EDELC(t) [description = "Excess Demand Effect on Life of Capacity"]
    @variables ETFP(t) = inits[:ETFP] [description = "Embedded TFP"]
    ...
    @variables OWELC(t) [description = "Observed Warming Effect on Life of Capacity"]
    @variables PEDE(t) = 1 [description = "Perceived Excess DEmand"]
