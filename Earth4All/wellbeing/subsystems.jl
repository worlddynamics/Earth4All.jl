...
function wellbeing(; name, params=_params, inits=_inits, tables=_tables, ranges=_ranges)
    ...
    @parameters GWEAWBGWF = params[:GWEAWBGWF] [description = "sGWeoAW<0<0: Global Warming Effect on Average WellBeing from Global Warming Flag"]
    ...
    @variables PAWBI(t) = inits[:PAWBI] [description = "Past Awerage WellBeing Index"]
